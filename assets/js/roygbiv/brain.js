var Brain = function(kwargs) {
	...
	*/
	var direction = "left"; //because its late and this wont hurt anything
	var _this = this;
...
/*	function render() {
	...
	this.animate = function() {
		...
		this.rotateBrain(this.camera, this.scene);
	...
	this.selectMeshByRandomBest = function() {  // CURT ADDED
		var bestMeshes = ["1027","1020", "1019", "1003", "1018", "1024", "1022", "1031", "1030", "1015", "1008", "1009", "1011"];
		...
		var meshNum = bestMeshes[ Math.floor(Math.random() * bestMeshes.length ) ];
		console.log("Mesh num: " + meshNum);

		for (var i in _this.meshes) {
			if (_this.meshes[i].roi_key == meshNum) {
				return _this.meshes[i];
			}
		}
	}

	this.selectMeshByRandom = function() {  // CURT ADDED
		/* Returns a random mesh */
		console.log("in select by random");
		console.log("meshes obj: " + Object.values(_this.meshes) );
		var meshNum = Math.floor(Math.random() * 35) + 1000;
		console.log("Mesh num: " + meshNum);

		for (var i in _this.meshes) {
			if (_this.meshes[i].roi_key == meshNum.toString() ) {
				return _this.meshes[i];
			}
		}
	}
	...
	this.rotateBrain = function(camera, scene) {  // CURT ADDS ROTATION
		...
		if(camera.position.x > 30){ direction = "right"; }
		if(camera.position.x < -30){ direction = "left"; }
	...
	this.selectMeshByMouse = function(e) {
		...
		if (intersects.length == 0) {
			...
			console.log("picked mesh #: " + picked_key);
